 ...
 * @author zhu
 */  
public class EducationCrawler extends BreadthCrawler {  
    ...
    public EducationCrawler(String crawlPath, boolean autoParse,HashMap<String, String> aWebsite) {  
...
//				System.out.println("时间：" + time);
				CatchAndStorageEducationNews.finalWork(url, website);
...
//    }  
    ...
    public static void startCrawler(HashMap<String, String> website,int maxUrlNum,int floodNum,int threadNum ) throws Exception {  
    	...
    	//判断这个网站的文件夹是否存在，不存在的话创建
    	File aFile = new File("d:/inews/htmlfile/"+website.get("english"));
    	if(!aFile.exists()){
    		aFile.mkdir();
    	}
    	
        EducationCrawler crawler = new EducationCrawler("crawl", true,website);  
